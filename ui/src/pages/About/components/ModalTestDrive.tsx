...
const ModalTestDrive = ({carId, open, setModal}) => {
    const steps = ['Оберіть дату тестдрайву', 'Оберіть філіал'];
    ...
    const createTestDrive = () => {
        setLoading(true);
        const api = new Api();
        const data = {car_id: carId, date: moment(selectedDate).unix(), dealer_center_id: selectedFilial} as PostTestDrive;
